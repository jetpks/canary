# Canary eval sweep

actual spend (from recorded token usage x price table): $0.0000

spend guard cap derivation:
  qwen3.8-flash-next-ream288                     132 calls x 16384 x $0.00000000 = $0.0000
  worst case sum: $0.0000
  cap (3x worst case, rounded up): $0

## run config

- **schema_version**: `3`
- **generated_at**: `2026-09-01T17:14:23Z`
- **tasks**: `44`
- **hidden_k**: `3`
- **visible_k**: `1`
- **max_tokens**: `16384`
- **temperature**: `1.0`
- **seed_derivation**: `sha256(prompt + sample_index)[0,8]`
- **concurrency**: `{"qwen3.8-flash-next-ream288":1}`
- **providers**: `{"qwen3.8-flash-next-ream288":"studio"}`
- **extra_body**: `{"qwen3.8-flash-next-ream288":{"reasoning_effort":"none"}}`

system prompt (schema 3+; v2 runs had none, so their rates are not poolable with these):

```text
You are completing a self-contained Ruby coding task.

Reply with exactly one fenced Ruby code block containing the complete
implementation, and nothing else - no prose before or after it.

You have no tools, no filesystem access, and no shell. Do not emit
tool calls; write the code directly.
```

## qwen3.8-flash-next-ream288 / hidden

### authored

- scored: 101, non_score: 13
- non_scores_by_reason: {extractor_refusal: 8, premature_stop: 5}
- pass_at_1: 0.545045045045045 (tasks_counted: 37)
- wall_clock: total 3277.3s, median/sample 6.4s, per_task_passed 117.0s
- pass_at_3: 0.8214285714285714 (tasks_counted: 28)

| task | scored | passed | non_score_reasons |
|---|---|---|---|
| block_memoizer | 3 | 0 | {} |
| block_safe_caller | 3 | 2 | {} |
| case_match_temperature_band | 2 | 0 | {premature_stop: 1} |
| character_safe_truncator | 2 | 1 | {extractor_refusal: 1} |
| comparable_money | 3 | 3 | {} |
| coordinate_destructure_splat | 3 | 0 | {} |
| credits_reel_contributor_chain | 3 | 3 | {} |
| delegating_proxy_method_object | 3 | 2 | {} |
| delivery_audited_route_block | 3 | 2 | {} |
| distance_coerce_subtraction | 2 | 0 | {premature_stop: 1} |
| ensure_return_swallows_exception | 3 | 0 | {} |
| enumerable_sparse_array | 3 | 2 | {} |
| enumerator_lazy_short_circuit | 2 | 2 | {extractor_refusal: 1} |
| enumerator_without_block_countdown | 3 | 1 | {} |
| eql_hash_distance_point | 2 | 0 | {premature_stop: 1} |
| event_broadcaster_duck_protocol | 3 | 3 | {} |
| exception_retrier | 3 | 2 | {} |
| expense_tracker_injected_rounding | 2 | 1 | {premature_stop: 1} |
| hash_default_proc_tally | 3 | 1 | {} |
| identity_registry | 3 | 1 | {} |
| keyword_splat_invoker | 3 | 0 | {} |
| ledger_freeze_dup_clone | 3 | 1 | {} |
| metaprogramming_open_record | 2 | 0 | {premature_stop: 1} |
| mood_board_owned_swatches | 3 | 3 | {} |
| numeric_coerce_fraction | 3 | 1 | {} |
| peekable_token_stream | 3 | 3 | {} |
| playlist_dup_clone_singleton | 0 | 0 | {extractor_refusal: 3} |
| protected_interval_overlap | 3 | 3 | {} |
| reading_circle_member_enumeration | 2 | 1 | {extractor_refusal: 1} |
| shadowed_constant_circle | 3 | 3 | {} |
| shipment_alert_recipient_protocol | 3 | 3 | {} |
| shipping_quote_polymorphic_packages | 3 | 3 | {} |
| string_attr_parser | 3 | 0 | {} |
| struct_vector | 3 | 1 | {} |
| tallying_key_store_fetch_contract | 3 | 1 | {} |
| temperature_report_injected_scale | 3 | 3 | {} |
| to_proc_lookup | 1 | 1 | {extractor_refusal: 2} |
| weight_unit_partial_order | 3 | 3 | {} |

### sourced

- scored: 16, non_score: 2
- non_scores_by_reason: {extractor_refusal: 2}
- pass_at_1: 0.888888888888889 (tasks_counted: 6)
- wall_clock: total 395.2s, median/sample 5.2s, per_task_passed 65.9s
- pass_at_3: 1.0 (tasks_counted: 4)

| task | scored | passed | non_score_reasons |
|---|---|---|---|
| attribute_bag_hash_equality | 3 | 2 | {} |
| cache_sweep_packed_entries | 3 | 2 | {} |
| counter_override_dup_clone | 2 | 2 | {extractor_refusal: 1} |
| gauge_reading_exact_sum | 2 | 2 | {extractor_refusal: 1} |
| stock_count_float_difference | 3 | 3 | {} |
| text_squash_no_mutation | 3 | 3 | {} |
