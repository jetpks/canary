# Canary eval sweep

actual spend (from recorded token usage x price table): $0.0000

spend guard cap derivation:
  ministral-3-8b                                 132 calls x 16384 x $0.00000000 = $0.0000
  worst case sum: $0.0000
  cap (3x worst case, rounded up): $0

## run config

- **schema_version**: `3`
- **generated_at**: `2026-09-01T22:35:19Z`
- **tasks**: `44`
- **hidden_k**: `3`
- **visible_k**: `1`
- **max_tokens**: `16384`
- **temperature**: `1.0`
- **seed_derivation**: `sha256(prompt + sample_index)[0,8]`
- **concurrency**: `{"ministral-3-8b":1}`
- **providers**: `{"ministral-3-8b":"studio"}`
- **extra_body**: `{"ministral-3-8b":{}}`

system prompt (schema 3+; v2 runs had none, so their rates are not poolable with these):

```text
You are completing a self-contained Ruby coding task.

Reply with exactly one fenced Ruby code block containing the complete
implementation, and nothing else - no prose before or after it.

You have no tools, no filesystem access, and no shell. Do not emit
tool calls; write the code directly.
```

## ministral-3-8b / hidden

### authored

- scored: 109, non_score: 5
- non_scores_by_reason: {premature_stop: 5}
- pass_at_1: 0.4035087719298246 (tasks_counted: 38)
- wall_clock: total 1391.5s, median/sample 1.7s, per_task_passed 60.5s
- pass_at_3: 0.696969696969697 (tasks_counted: 33)

| task | scored | passed | non_score_reasons |
|---|---|---|---|
| block_memoizer | 3 | 2 | {} |
| block_safe_caller | 2 | 0 | {premature_stop: 1} |
| case_match_temperature_band | 3 | 2 | {} |
| character_safe_truncator | 3 | 1 | {} |
| comparable_money | 3 | 2 | {} |
| coordinate_destructure_splat | 3 | 1 | {} |
| credits_reel_contributor_chain | 3 | 3 | {} |
| delegating_proxy_method_object | 2 | 0 | {premature_stop: 1} |
| delivery_audited_route_block | 3 | 0 | {} |
| distance_coerce_subtraction | 3 | 0 | {} |
| ensure_return_swallows_exception | 3 | 2 | {} |
| enumerable_sparse_array | 3 | 0 | {} |
| enumerator_lazy_short_circuit | 2 | 0 | {premature_stop: 1} |
| enumerator_without_block_countdown | 3 | 2 | {} |
| eql_hash_distance_point | 3 | 2 | {} |
| event_broadcaster_duck_protocol | 3 | 3 | {} |
| exception_retrier | 3 | 1 | {} |
| expense_tracker_injected_rounding | 3 | 2 | {} |
| hash_default_proc_tally | 3 | 3 | {} |
| identity_registry | 3 | 0 | {} |
| keyword_splat_invoker | 3 | 2 | {} |
| ledger_freeze_dup_clone | 3 | 0 | {} |
| metaprogramming_open_record | 3 | 0 | {} |
| mood_board_owned_swatches | 3 | 3 | {} |
| numeric_coerce_fraction | 2 | 0 | {premature_stop: 1} |
| peekable_token_stream | 3 | 1 | {} |
| playlist_dup_clone_singleton | 2 | 0 | {premature_stop: 1} |
| protected_interval_overlap | 3 | 1 | {} |
| reading_circle_member_enumeration | 3 | 0 | {} |
| shadowed_constant_circle | 3 | 3 | {} |
| shipment_alert_recipient_protocol | 3 | 3 | {} |
| shipping_quote_polymorphic_packages | 3 | 3 | {} |
| string_attr_parser | 3 | 0 | {} |
| struct_vector | 3 | 0 | {} |
| tallying_key_store_fetch_contract | 3 | 0 | {} |
| temperature_report_injected_scale | 3 | 2 | {} |
| to_proc_lookup | 3 | 1 | {} |
| weight_unit_partial_order | 3 | 1 | {} |

### sourced

- scored: 17, non_score: 1
- non_scores_by_reason: {premature_stop: 1}
- pass_at_1: 0.4444444444444445 (tasks_counted: 6)
- wall_clock: total 160.5s, median/sample 2.2s, per_task_passed 40.1s
- pass_at_3: 0.8 (tasks_counted: 5)

| task | scored | passed | non_score_reasons |
|---|---|---|---|
| attribute_bag_hash_equality | 3 | 2 | {} |
| cache_sweep_packed_entries | 2 | 0 | {premature_stop: 1} |
| counter_override_dup_clone | 3 | 0 | {} |
| gauge_reading_exact_sum | 3 | 2 | {} |
| stock_count_float_difference | 3 | 3 | {} |
| text_squash_no_mutation | 3 | 1 | {} |
