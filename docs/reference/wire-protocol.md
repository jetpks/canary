...
| `stdout`, `stderr` | Object | `{text, truncated}` — output captured in the child (`$stdout`/`$stderr` are swapped for the life of the eval). `text` is capped at `Canary::Pool::EVAL_OUTPUT_LIMIT` bytes (64 MiB); `truncated` is `true` when the real output was longer, mirroring `value`'s own `truncated` flag. Reported for `"ok"` and `"raised"`. A `"timeout"` or `"crash"` child reports empty stream objects (`text: ""`, `truncated: false`): the result ships to the parent only on the child's own completion (the relay's Marshal write), and a killed or crashed child never gets there — this is inherent to the relay, not eval-specific. |
